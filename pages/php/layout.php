...
function pageStart(string $title, array $user): void
{
    ...
    if (in_array($user['perfil'], ['medico', 'enfermeiro'], true)) {
        $extraLink = ' <a href="../int/index.php">Internações</a>';
    }
    echo '<div class="app-shell"><header class="app-header"><div class="app-header__title"><h1>' . e($title) . '</h1><p>Usuário: ' . e($user['nome']) . ' | Perfil: ' . e($user['perfil']) . '</p></div><nav class="app-nav"><a href="../' . e(redirectToProfile($user['perfil'])) . '">Início</a>' . $extraLink . ' <a href="../logout.php">Sair</a></nav></header><main class="app-main">';
}
